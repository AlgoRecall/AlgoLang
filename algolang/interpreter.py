...
class Interpreter:
    ...
    def visit_for_statement(self, node: ast.ForStatement) -> None:
        ...
        try:
            for index, value in enumerate(values):
                ...
                try: self._execute_block(node.body, self._new_environment(loop_environment))
                except _Continue: continue
                except _Break:
                    reason = LoopExitReason.BREAK
                    break
    ...
    def visit_binary_expression(self, node: ast.BinaryExpression) -> RuntimeValue:
        ...
        try:
            ...
            if node.operator == "-": return left - right
            if node.operator == "*": return left * right
            if node.operator == "/": return self._integer_quotient(left, right) if isinstance(left, int) and isinstance(right, int) else left / right
            if node.operator == "%":
                return left - self._integer_quotient(left, right) * right if isinstance(left, int) and isinstance(right, int) else left % right
            if node.operator == "==": return left == right
            if node.operator == "!=": return left != right
            if node.operator == "<": return left < right
            if node.operator == "<=": return left <= right
    ...
    def visit_call_expression(self, node: ast.CallExpression) -> RuntimeValue:
        ...
        if isinstance(callee, NativeFunction):
            try: return callee.function(*arguments)
            except (IndexError, KeyError, TypeError, ValueError) as error: self._error(node, f"{callee.name} failed: {error}")
    ...
    def _call_function(self, function: AlgoFunction, arguments: list[RuntimeValue], node: ast.Node) -> RuntimeValue:
        ...
        result = None
        try:
            try: self._execute_block(declaration.body, self._new_environment(environment))
            except _Return as returned: result = returned.value
    ...
    def _iterable(self, value: RuntimeValue, node: ast.Node) -> list[RuntimeValue]:
        ...
        except TypeError: self._error(node, f"{type_name(value)} is not iterable")
